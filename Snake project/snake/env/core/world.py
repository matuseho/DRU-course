...
class World(object):
    ...
    def init_food(self):
        ...
        available_food_positions = self.available_food_positions.difference(self.snake.blocks)
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if not (0 <= new_snake_head[0] < self.size[0]) or not(0 <= new_snake_head[1] < self.size[1]) \
                    or self.world[new_snake_head[0], new_snake_head[1]] == self.WALL:
